 ...
 */
#include <MycilaDimmerBurstFire.h>
...
#define TAG "BurstFire"

struct RegisteredDimmer;
struct RegisteredDimmer {
    Mycila::BurstFireDimmer* dimmer = nullptr;
...
void Mycila::BurstFireDimmer::begin() {
  if (_enabled)
    return;
...
void Mycila::BurstFireDimmer::end() {
  if (!_enabled)
    return;
  _enabled = false;
  _online = false;
...
bool Mycila::BurstFireDimmer::_apply() {
  if (!_enabled)
    return false;
...
void ARDUINO_ISR_ATTR Mycila::BurstFireDimmer::onZeroCross(int16_t delayUntilZero, void* arg) {
...
bool ARDUINO_ISR_ATTR Mycila::BurstFireDimmer::_fireTimerISR(gptimer_handle_t timer, const gptimer_alarm_event_data_t* event, void* arg) {
...
void Mycila::BurstFireDimmer::_registerDimmer(Mycila::BurstFireDimmer* dimmer) {
  if (dimmers == nullptr) {
...
void Mycila::BurstFireDimmer::_unregisterDimmer(Mycila::BurstFireDimmer* dimmer) {
