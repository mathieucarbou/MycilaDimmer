...
namespace Mycila {
  class BurstFireDimmer : public Dimmer {
    public:
      virtual ~BurstFireDimmer() { end(); }
      ...
      void setSemiPeriod(uint16_t semiPeriod) {
        if (semiPeriod == 0) {
          ESP_LOGE("BurstFire", "setSemiPeriod: semiPeriod must be > 0");
        }
        assert(semiPeriod > 0);
      ...
      const char* type() const override { return "burst-fire"; }
    ...
    private:
      ...
      bool _running = false;
      // bool _fire = false; // tells whether the dimmer should fire for the next half-cycle

      static bool _fireTimerISR(gptimer_handle_t timer, const gptimer_alarm_event_data_t* event, void* arg);
      static void _registerDimmer(Mycila::BurstFireDimmer* dimmer);
      static void _unregisterDimmer(Mycila::BurstFireDimmer* dimmer);
  };
} // namespace Mycila
